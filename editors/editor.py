...
class BaseEditor:
    ...
    def edit(self,
        ...
        if "requests" in kwargs.keys():
            ...
            requests = _prepare_requests(prompts, target_new, ground_truth, target_neg, rephrase_prompts, locality_inputs, portability_inputs, subject, **kwargs)
    ...
    def edit_requests(self,
        ...
        # if sequential_edit:
        #     #
        #     # all_metrics = [a for a, _ in sorted(zip(all_metrics, acc_sorts), key=lambda x: x[1], reverse=True)] #
        #     # requests = [a for a, _ in sorted(zip(requests, acc_sorts), key=lambda x: x[1], reverse=True)]
        #     sorted_idx = None
        #     if self.hparams.grad_sort:
        #         edited_model, weights_copy, icl_examples, sorted_idx = edit_func(requests)
        #     elif self.alg_name == 'WISE':
        #         for i, request in enumerate(tqdm(requests, total=len(requests))):
        #             edited_model, weights_copy, icl_examples, sorted_idx = edit_func([request])
        #     else:
        #         for i, request in enumerate(tqdm(requests, total=len(requests))):
        ...
        if sequential_edit:
            ...
            # requests = [a for a, _ in sorted(zip(requests, acc_sorts), key=lambda x: x[1], reverse=True)]
            sorted_idx = None
            if hasattr(self.hparams, 'grad_sort') and self.hparams.grad_sort:
                edited_model, weights_copy, icl_examples, sorted_idx = edit_func(requests)
            elif self.alg_name == 'WISE':
                for i, request in enumerate(tqdm(requests, total=len(requests))):
                    edited_model, weights_copy, icl_examples = edit_func([request])
            ...
            # torch.save(edited_model.edit_module.down_proj.cluster_activation, '/root/zzg/easyedit0427/wyl_run_output/pth/clu_act.pth')
            # torch.save(edited_model.edit_module.down_proj.cluster_bound, '/root/zzg/easyedit0427/wyl_run_output/pth/clu_bnd.pth')
            # if sorted_idx is not None:
            #     for i, request in enumerate(requests):
            #         edit_evaluation(all_metrics, request, edited_model, i, test_generation, icl_examples, **kwargs)
            for i, request in enumerate(requests):
        ...
        if self.alg_name == 'WISE':
            return all_metrics, edited_model, weights_copy, sorted_idx
